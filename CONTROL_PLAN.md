...
- **Parachute**: `Z` toggles the parachute while airborne.
...
## Contextual Action Menu (Hold `A`)
- Holding `A` opens a radial action overlay; release while highlighting an option to trigger it.
- Quick-tapping `A` repeats the last action (tracked separately per movement mode) if still valid.
- Use arrow keys to cycle options; releasing `A` or pressing `space` confirms the highlighted action.
...
## Availability Rules
- The menu rebuilds each time `A` is pressed using capability checks.
- Disabled entries are not visible
- Options stay hidden until the player actually obtains the underlying upgrade.

## HUD & UX Notes
- The favourite action indicator shows which option a quick `A` tap will trigger next.
